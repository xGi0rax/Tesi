...
\noindent L'interazione tra l'applicazione e il database non avviene tramite query testuali dirette, ma è mediata da un framework \gls{ormg}. Nello specifico, per il contesto mobile è stata adottata la libreria open-source \texttt{sqlite-net-pcl} (\cite{site:sqlite-net-pcl}), un framework leggero e ottimizzato per \gls{dotnetmauig}. Questa scelta progettuale ha permesso di mappare automaticamente i modelli dati descritti in precedenza (le classi \gls{csharpg}) nelle rispettive tabelle relazionali. L'utilizzo di questo specifico \gls{orm} eleva il livello di astrazione: il team di sviluppo può manipolare i record del database come se fossero normali oggetti in memoria, delegando al framework la traduzione di queste operazioni nel linguaggio \textit{SQL} sottostante. Ciò riduce drasticamente la probabilità di errori sintattici e facilita la manutenzione futura.
