...
\noindent L'interazione tra l'applicazione e il database non avviene tramite query testuali dirette, ma è mediata da una libreria \gls{ormg}. Nello specifico, per il contesto mobile è stata adottata la libreria open-source \texttt{sqlite-net-pcl} (\cite{site:sqlite-net-pcl}), una soluzione leggera e ottimizzata per \gls{dotnetmauig}. Questa scelta progettuale ha permesso di mappare tramite attributi i modelli dati descritti in precedenza (le classi \gls{csharpg}) nelle rispettive tabelle relazionali del database \gls{sqliteg}. L'utilizzo di tale libreria eleva il livello di astrazione: il team di sviluppo può manipolare i record del database come se fossero normali oggetti in memoria, delegando alla libreria la traduzione delle operazioni di persistenza nel linguaggio \textit{SQL} sottostante. Ciò riduce drasticamente la probabilità di errori sintattici e semplifica la manutenzione e l'evoluzione futura del codice.
