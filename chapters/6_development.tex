...
Di seguito viene riportato un frammento di codice della classe \texttt{ErgDatabase} relativo ai principali metodi di inserimento e aggiornamento dei record. Come si può notare, ogni singola operazione sul database sfrutta il costrutto \texttt{lock} applicato all'oggetto statico \texttt{\_locker}. Questa architettura garantisce l'esecuzione sequenziale delle transazioni, mettendo in coda eventuali richieste simultanee ed evitando collisioni o corruzioni del file fisico del database:
\begin{listing}[H]
\begin{minted}{c}
