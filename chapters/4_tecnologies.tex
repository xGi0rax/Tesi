...
\begin{itemize}
    ...
    \item \textbf{Estrazione automatica dei dati:} per velocizzare l'inserimento, il sistema deve interfacciarsi con l'hardware del dispositivo (la fotocamera) e comunicare con un servizio cloud di \gls{ia} per l'analisi e l'estrazione del testo dai giustificativi cartacei allegati.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Unificazione del linguaggio:} l'\gls{erp} e i web services aziendali di Ergon sono storicamente sviluppati utilizzando l'ecosistema Microsoft e il linguaggio \gls{csharpg}. Utilizzare MAUI ha permesso di mantenere una coerenza tecnologica tra \textit{frontend} e \textit{backend}.
...
\noindent Per la gestione della persistenza locale è stato scelto \gls{sqliteg} (\cite{site:sqlite}), un database relazionale \gls{embeddedg} ampiamente utilizzato nello sviluppo mobile, il cui logo è riportato in Figura \ref{fig:sqlite}. La scelta è motivata da diversi fattori:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \label{fig:rest-api}
\end{figure}
\noindent Come illustrato nello schema in Figura \ref{fig:rest-api}, per consentire all'applicazione mobile di dialogare con il server aziendale e sincronizzare le note spesa, è necessario implementare un'architettura di rete basata sui web services. La comunicazione tra i diversi componenti software distribuiti avviene tramite \gls{api}, le quali definiscono i protocolli, le regole di richiesta e le strutture di risposta. Nello specifico, lo standard di fatto adottato è lo stile architetturale REST, il quale impone una serie di vincoli progettuali per la comunicazione basata sulle richieste attraverso i metodi HTTP standard, come \textit{GET}, \textit{POST}, \textit{PUT} e \textit{DELETE}. Il trasferimento delle informazioni avviene scambiando pacchetti testuali nel formato \textit{JSON} (\cite{site:json}), scelto per la sua enorme versatilità, leggerezza e per la capacità di essere elaborato facilmente dalle librerie standard.  

\subsection*{Comunicazione con l'ERP aziendale}
\label{subsec:comunicazione-erp}
\noindent Nel contesto del progetto, le \gls{apirestg} rappresentano il punto di integrazione tra l'applicazione mobile e il sistema \gls{erp} aziendale. L'applicazione invia al backend le informazioni relative alle note spesa e riceve i dati necessari alla compilazione e alla validazione delle operazioni. \newline 
Per la realizzazione di questi servizi, in linea con lo standard tecnologico aziendale, è stato utilizzato il framework \gls{aspnetwebapig} (\cite{site:aspnet-webapi}), il quale consente di creare servizi web \textit{RESTful} in modo efficiente. Inoltre, per gestire l'accesso ai dati e la persistenza sul database relazionale centrale, è stato adottato \gls{entityframeworkg} (\cite{site:ef6}), garantendo sicurezza e integrità delle transazioni. Questo approccio consente di mantenere separati il livello di presentazione e le logiche di business del gestionale, favorendo la manutenibilità, la sicurezza e l'evoluzione indipendente dei diversi componenti del sistema.
