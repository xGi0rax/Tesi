...
\begin{itemize}
    \item \textbf{Dipendente (Utente):} rappresenta un dipendente dell'azienda che ha già effettuato l'accesso all'applicazione \gls{ergteam} tramite le proprie credenziali. Esso interagisce con la nuova sezione per consultare lo storico delle proprie spese inserite e per inserire, allegando evenutali fotografie, nuove note spesa da inoltrare al sistema.
    
    \item \textbf{Azure Content Understanding (Sistema di \gls{ia}):} sistema esterno basato su intelligenza artificiale, interrogato dall'applicazione per l'estrazione automatica dei dati dalle fotografie degli scontrini, al fine di semplificare e velocizzare la compliazione della nota spesa da parte del dipendente.
\end{itemize}
...
\begin{usecase}{4.1}{Inserimento nuova nota spesa}
    ...
    \usecasealt{
        \begin{itemize}
            ...
            \item \textbf{Elaborazione automatica:} il sistema estrae i dati dalla fotografia tramite il modulo di intelligenza artificiale e li propone all'utente (\ref{uc:uc_analisi_foto_scontrino}).
...
\begin{usecase}{4.3}{Inserimento nota sincronizzata}
    \usecaseactors{Utente.}
    \usecasedesc{L'utente vuole inserire una nuova nota spesa sincronizzata nel sistema.}
    \usecasepre{L'utente ha effettuato l'accesso alla schermata di inserimento di una nuova nota spesa.}
    \usecasepost{La nuova nota spesa sincronizzata viene salvata nel sistema.}
    \usecaseprin{
        \begin{itemize}
            \item L'utente inserisce tutti i campi obbligatori per la creazione di una nuova nota spesa.
            \item L'utente non seleziona l'opzione "Posticipa caricamento".
            \item L'utente preme il pulsante "SALVA NOTA" per confermare l'inserimento.
        \end{itemize}
    }
    \label{uc:uc_inserimento_nota_sincronizzata}
\end{usecase}

\begin{usecase}{4.4}{Inserimento nota sincronizzata offline}
    \usecaseactors{Utente.}
    \usecasedesc{L'utente vuole inserire una nuova nota spesa sincronizzata nel sistema.}
    ...
    \label{uc:uc_inserimento_nota_sincronizzata_offline}
\end{usecase}

\begin{usecase}{4.5}{Visualizzazione errore validazione dati}
    \usecaseactors{Utente.}
...
\begin{usecase}{5}{Compilazione dati generali spesa.}
    ...
    \usecaseprin{
        \begin{itemize}
            ...
            \item L'utente può allegare la foto dello scontrino (\ref{uc:uc_inserimento_foto_scontrino}).
        \end{itemize}
    }
...
\begin{usecase}{8}{Pop-up errore estrazione.}
    \usecaseactors{Utente, Sistema \gls{ia}.}
    ...
    \usecaseprin{
        \begin{itemize}
            \item La connessione è assente oppure il sistema \gls{ia} non riesce ad estrarre i dati dalla foto allegata.
...
\begin{usecase}{9}{Pop-up verifica importo estratto.}
    \usecaseactors{Utente.}
...
\begin{usecase}{10}{Pop-up verifica data estratta.}
    \usecaseactors{Utente.}
...
\begin{usecase}{11}{Visualizzazione dati estratti.}
    \usecaseactors{Utente, Sistema \gls{ia}.}
...
\begin{usecase}{12}{Rimozione foto allegata.}
    \usecaseactors{Utente.}
...
\begin{usecase}{13}{Inserimento multiplo di spese}
    \usecaseactors{Utente.}
...
\begin{usecase}{14}{Visualizzazione voce riepilogo spese}
    \usecaseactors{Utente.}
    ...
    \usecaseprin{
        \begin{itemize}
            ...
            \item Il sistema espande la sezione nell'interfaccia corrente.
            \item Per ogni spesa inserita vengono mostrati: tipologia, data e importo.
        \end{itemize}
    }
    \label{uc:uc_visualizzazione_voce_riepilogo_spese}
\end{usecase}

\begin{usecase}{15}{Visualizzazione riepilogo spese}
    \usecaseactors{Utente.}
...
\begin{usecase}{16}{Pop-up promemoria sincronizzazione}
    \usecaseactors{Utente.}
...
\begin{usecase}{17}{Visualizzazione dettaglio mensile}
    \usecaseactors{Utente.}
...
\begin{usecase}{18}{Accesso schermata modifica}
    \usecaseactors{Utente.}
...
\begin{usecase}{19}{Modifica nota spesa}
    \usecaseactors{Utente.}
...
\begin{usecase}{20}{Visualizzazione nota in sola lettura}
    \usecaseactors{Utente.}
...
\begin{usecase}{21}{Eliminazione nota spesa locale}
    \usecaseactors{Utente.}
...
\begin{usecase}{22}{Sincronizzazione singola nota locale}
    \usecaseactors{Utente.}
...
\begin{center}
    ...
    RFO-4 & Il sistema deve validare i dati inseriti e impedire il salvataggio in caso di informazioni mancanti o errate. & UC4.5, UC5.3 \\ \hline
    RFO-5 & L'utente deve poter aggiungere e accodare transazioni multiple all'interno di un singolo inserimento di nota spesa. & UC13, UC14, UC15 \\ \hline
    RFO-6 & Il sistema deve gestire il salvataggio locale (offline) delle note in caso di assenza di rete o di scelta volontaria. & UC4.2, UC4.4 \\ \hline
    RFO-7 & L'utente deve poter sincronizzare in blocco o singolarmente le note salvate in locale. & UC3, UC22 \\ \hline
    RFO-8 & L'utente deve poter le note spesa appartenenti a un singolo resoconto mensile & UC17 \\ \hline
    RFO-9 & L'utente deve poter eliminare una nota spesa esistente, purché non sia storicizzata. & UC21 \\ \hline
    RFO-10 & Il sistema deve impedire la modifica (sola lettura) delle note spesa antecedenti alla data minima consentita. & UC20 \\ \hline
    RFD-1 & L'utente deve poter allegare una fotografia dello scontrino per estrarne automaticamente i dati tramite \gls{ia}. & UC7, UC7.1 \\ \hline
    RFD-2 & Il sistema deve gestire i conflitti tra i dati inseriti manualmente e quelli estratti dall'\gls{ia} tramite pop-up. & UC8, UC9, UC10 \\ \hline
    RFD-3 & Il sistema deve ricordare all'utente, all'avvio del modulo, la presenza di note locali non sincronizzate. & UC16 \\
    RFD-4 & L'utente deve poter accedere alla modalità modifica di una nota spesa. & UC18 \\ \hline
    RFD-5 &  L'utente deve poter modificare una nota spesa esistente. & UC19 \\ \hline
...
\begin{center}
    ...
    RQO-3 & È necessario realizzare opportuni test di unità e accettazione per validare le funzionalità dell'applicazione. & - \\ \hline
...
\begin{center}
    ...
    RVO-6 & I dati delle note spesa inviati al \textit{backend} devono integrarsi con l'ERP aziendale per la gestione amministrativa. & - \\ \hline
    RVD-1 & Per il servizio di estrazione dati dalle immagini deve essere interrogata l'API di Azure Content Understanding. & - \\
    \hline
    \hiderowcolors
